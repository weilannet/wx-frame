<template>
  <div>
    ...
    <scroller lock-x scrollbar-y use-pullup use-pulldown height="500px" :pullup-config="pullupConfig2" @pullup:loading="loadMore" @pulldown:loading="refresh" :pullup-status.sync="pullupStatus" >
      ...
      <!--</div>-->
      <div class="box2">
      <group class="myreport-item" v-for="item in reportlist" track-by="Math.random()*1000+id" @click="itemClick(item)">
  ...
  export default {
    ...
    methods: {
      bindData () {
        ...
        if (this.pageindex === 1) {
          this.$set('reportlist', result)
          return
        }
        this.$set('reportlist', this.reportlist.concat(result))
      },
      tabClick (item) {
        this.tabselect = item
      },
